...
<html lang="ko-KR">
    <head>
        ...
        <title>비밀번호 찾기 - 가즈-아(GAZ-AAA)</title>
        <?php
            session_start();
            ...
            if(empty($UserID)) { echo "<script>alert('잘못된 접근입니다.'); history.back();</script>"; exit(); }
            mysqli_select_db($Connect, "user");
            $Query = "SELECT * FROM userlist WHERE ID='$UserID' and Phone='$UserPhone'";
            $Result = mysqli_query($Connect, $Query);
            $UserID = "";
            while($Rows = mysqli_fetch_array($Result)) $UserID = $Rows['ID'];
            if($UserID == "") echo "<script>alert('아이디를 찾을 수 없습니다.'); history.back();</script>";
        ?>
    </head>
    <body>
        <div class="Header">
        ...
        <div class="FindID_Window">
            <div class="InputForm">
                <div id="FindID"><h2>본인 인증</h2></div>
                <form action="FindPW-3.php" method="POST">
                    <div class="FindID">
