...
int main()
{
    ...
    if (mysql_real_connect(Connect, HOST, HOSTNAME, HOSTPW, DB, PORT, NULL, 0)) // DB 연결 성공 
    {
        ...
        sprintf(Query, "%s", "SET names euckr");
        ...
        for (int i = 0; i < 40; i++) // 1시간 단위 불러오기 - 40시간 
        {
            hour -= 1;
            if (hour < 0) // 0시 이전은 조회 X
            {
        ...
        {
            timer = time(NULL); // 1970년 1월 1일 0시 0분 0초부터 시작하여 현재까지의 초 
            localtime_s(&t, &timer); // 포맷팅을 위해 구조체에 넣기(변경된 시간 변수 초기화) 
...
void NewCardIssue(char* CoinName, char* CoinNews, int* NewsContinuous, int* NewsEffect)
{
    ...
    else if (strcmp(CoinName, "서브프레임") == 0 && *NewsContinuous == 0)
    {
        ...
        {
        case 0:
            sprintf(CoinNews, "%s", "서브프레임, 블록체인 메신저 중 보안성이 매우 뛰어나...");
            *NewsEffect = 3;
            break;
        case 1:
            sprintf(CoinNews, "%s", "서브프레임, 갑작스러운 서버 다운! 현재 원인 파악 중.");
            *NewsEffect = -4;
            break;
        case 2:
            sprintf(CoinNews, "%s", "서브프레임의 독자적인 블랙홀 라우팅 알고리즘 개발. “필요한 메시지만 식별 가능하다. 아직은 조금 더 지켜봐야...”");
            *NewsEffect = 4;
            break;
        case 3:
            sprintf(CoinNews, "%s", "사용자가 보낸 메시지를 악의적인 메시지로 인식해, 메시지 전송 오류 현상 발생! 이에 서브프레임은 “없어진 패킷은 복구 불가능하다.”");
            *NewsEffect = -3;
            break;
        default:
            sprintf(CoinNews, "%s", ""); // 3 ~ 4 뉴스 없음. 3 / 5 확률로 뉴스 
